...
public class GameControllerTest {
	...
	public void testCreateNewGame() throws Exception {
		...
		mockMvc.perform(post("/games")).andExpect(status().isCreated()).andExpect(jsonPath("$.id").value(1))
	...
	public void testMakeMove() throws Exception {
		...
		mockMvc.perform(put("/games/1/pits/3")).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(1))
